...
The `Caseinflu` package contains the procedure of case influence assessment, which is based on case influence graph (Add reference). For each observation $i^{*}$, the case influence graph is a function of case weight parameter $w$. At each $w > 0$, the function value is the "difference" between the full-data model and the case weight adjusted model (case influence measure), which can be formalized as 
\begin{align}
...
3. Local influence (Only for nonsmooth models) For more details, see section 5.1.3 of [Tu, S. (2019). *Case Influence and Model Complexity in Regression and Classification*](https://www.proquest.com/docview/2353041291?pq-origsite=gscholar&fromopenview=true).
